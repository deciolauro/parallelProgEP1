/** @file 	mandelbrot_omp.c
 *	@brief	OpenMP parallel program to compute and plot mandelbrot set
 *	
 *	C implementation of an OpenMP parallel program to compute and plot some
 *  subset of a mandelbrot set
 *
 *	Usage:
 *    ./mandelbrot_omp c_x_min c_x_max c_y_min c_y_max image_size ... 
 ...
 *
 *  Usage examples:
 *      Full Picture:         ./mandelbrot_omp -2.5 1.5 -2.0 2.0 11500 0
 *      Seahorse Valley:      ./mandelbrot_omp -0.8 -0.7 0.05 0.15 11500 2
 *      Elephant Valley:      ./mandelbrot_omp 0.175 0.375 -0.1 0.1 11500 5
 *      Triple Spiral Valley: ./mandelbrot_omp -0.188 -0.012 0.554 0.754 11500
 *
 *	Notes:
 ...
 *      	./mandelbrot_omp -2.5 1.5 -2.0 2.0 11500 4 -nt 4
 *		- 8 Threads using default color scheme:
 *          ./mandelbrot_omp -0.188 -0.012 0.554 0.754 11500 -nt 8
 *
...
void init(int argc, char *argv[])
{
    if(argc < 6)
	{
        printf("usage: ./mandelbrot_omp c_x_min c_x_max c_y_min c_y_max image_size color_scheme(optional default=0) ...\n");
        printf("examples with image_size = 11500:\n");
        printf("    Full Picture:         ./mandelbrot_omp -2.5 1.5 -2.0 2.0 11500 0\n");
        printf("    Seahorse Valley:      ./mandelbrot_omp -0.8 -0.7 0.05 0.15 11500 3\n");
        printf("    Elephant Valley:      ./mandelbrot_omp 0.175 0.375 -0.1 0.1 11500 -nt 12\n");
        printf("    Triple Spiral Valley: ./mandelbrot_omp -0.188 -0.012 0.554 0.754 11500 -nt 8\n");
        exit(0);
    }
    else
	{
...
void compute_mandelbrot()
{
    ...
    # pragma omp parallel \
    shared (iteration_max, i_x_max, i_y_max) \
    private (i_y, i_x, iteration, c_y, c_x, z_x, z_y, z_x_squared, z_y_squared)
    {
		#pragma omp for
